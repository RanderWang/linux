 ...
 * @cdns: Cadence instance
 */
int sdw_cdns_suspend(struct sdw_cdns *cdns, bool block_wake)
{
 ...
 * @cdns: Cadence instance
 */
int sdw_cdns_resume(struct sdw_cdns *cdns, bool block_wake)
{
	int ret;
	...
	if (ret < 0)
		dev_err(cdns->dev, "bus clock stop exit failed %d\n", ret);
